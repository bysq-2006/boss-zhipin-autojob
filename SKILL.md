...
脚本会: 先 `--wait` 再拉列表、解码薪资、算通勤、过滤、拟人点沟通; 本屏不够就自己滚左列继续,已投过和 `--after` 以上的卡不再给模型看。每次至少先检查 200 个岗位；如果当前没有匹配岗位，必须继续向下滚动并检查，直到岗位列表到底，或连续多次滚动后岗位数量不再增加、确认没有新增职位时才停止。未检查到 200 个岗位前，不得仅因当前屏幕没有匹配项就停止或通知用户。把 JSON `ok`/`results` 汇报即可。

## UIA 调用错误重试

如果 `apply` 或 `list` 的输出包含“UIA 调用错误”或等价的临时 UIA 异常，不得立即停止任务或向用户报告最终失败。等待随机 `3–5` 秒后重试刚才的同一命令，并重新选择 `0.3–3.0` 的 `--wait`。`apply` 重试时只使用尚未完成的投递数量，已由 JSON/投递记录确认成功的职位不得重复计数；已检查岗位数继续累计。持续重试，直到命令成功、用户明确叫停，或错误已明确变为“找不到窗口/标签页”这类非临时状态。UIA 临时错误本身不是启用 Computer Use 的理由。
